...
// return true if operation was successful, else false
bool array_is_equal(const void *array_a, void *array_b, const size_t elem_size,
                    const size_t elem_count);
...
// \param src_file the source file that contains the array to be read into the
// destination array
// \param dst_data the array that will contain the data stored inthe source file
// \param elem_size the number of bytes each array element uses of the
// destination array
...
// \param elem_size the number of bytes each array element uses and same as the
// target
